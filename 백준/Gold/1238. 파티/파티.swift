...
func input() -> (Int, Int, Int, [[(Int, Int)]]) {
    let vex = readLine()!.split(separator: " ").compactMap { Int(String($0)) }
    let (v, e, x) = (vex[0], vex[1], vex[2])
...
func solution() {
  let (v, _, x, graph) = input()
  var answer = 0
  for i in 1...v {
      let dist1 = dijkstra(n: v, graph: graph, start: i)
      let dist2 = dijkstra(n: v, graph: graph, start: x)
